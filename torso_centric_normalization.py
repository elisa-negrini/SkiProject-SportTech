...
OUTPUT_CSV = 'ski_dataset_normalized.csv' 
PLOT_OUTPUT_DIR = 'plots_verifica_adaptive'

# Punti Cardine (ID come stringhe nel JSON)
ANCHOR_IDS = {
    'neck': '2',        
    'r_shoulder': '3', 
    'l_shoulder': '6',  
...
def normalize_annotation_adaptive(ann, keypoint_names):
    ...
    # --- B. SCALA FISSA (SOLO BUSTO) ---
    
    # Stima Collo (Top) - Se manca il collo usa il centro delle spalle
    top_ref = neck
    ...
    # MODIFICA: Usiamo SOLO il busto come unità di misura.
    # Non calcoliamo più le spalle. Se il busto è schiacciato, 
    # le coordinate diventeranno grandi, mantenendo la coerenza 3D.
    scale_factor = torso_len
    
    # Protezione divisione per zero (o busto non calcolabile)
    if scale_factor < 1e-6: 
        return None

    # 4. Normalizzazione "Body Units"
    normalized_kpts = {}
...
def plot_skeleton_check(row, keypoint_names, skeleton_pairs, output_path):
    ...
    plt.title(f"Torso-Only Check: {row['jump_id']} - {row['frame_name']}")
