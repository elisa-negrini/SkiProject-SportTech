                ...
                if is_front or is_back:
                    ...
                    if all(x is not None for x in [p_tip_r, p_tail_r, p_tip_l, p_tail_l]):
                        ...
                        if is_back:
                            ...
                            # --- ROBUST SYMMETRY CALCULATION (WITH KNEE MIDPOINT) ---
                            ...
                            if all(p is not None for p in [p_neck, p_pelvis, p_knee_r, p_knee_l]):
                                # 1. Calcolo il punto medio delle ginocchia
                                p_mid_knee = (p_knee_r + p_knee_l) / 2
                                
                                # 2. Definisco l'asse: dal punto medio delle ginocchia verso il collo
                                # Questo asse è più rappresentativo dell'intera postura dell'atleta
                                vec_axis = p_mid_knee - p_neck 
                                
                                # 3. Calcolo gli angoli degli sci rispetto a questo nuovo asse
